...
from manager_rest.storage import get_node as get_storage_node, models
...
def evaluate_node(node):
    ...
    sm.get(models.Deployment, deployment_id, include=['id'])
    storage = FunctionEvaluationDBStorage(deployment_id, sm)
    try:
...
def evaluate_deployment_outputs(deployment_id):
    sm = get_storage_manager()
    deployment = sm.get(models.Deployment, deployment_id, include=['outputs'])
    storage = FunctionEvaluationDBStorage(deployment_id, sm)
...
def evaluate_deployment_capabilities(deployment_id):
    sm = get_storage_manager()
    deployment = sm.get(
        models.Deployment, deployment_id, include=['capabilities'])
    storage = FunctionEvaluationDBStorage(deployment_id, sm)
...
class FunctionEvaluationDBStorage(object):
    ...
    def get_node_instances(self, node_id=None):
        ...
        return [NodeInstance(inst.to_dict()) for inst in
                self.sm.list(models.NodeInstance, filters=filters).items]

    def get_node_instance(self, node_instance_id):
        ni = self.sm.get(models.NodeInstance, node_instance_id).to_dict()
        return NodeInstance(ni)

    def get_input(self, input_name):
        deployment = self.sm.get(models.Deployment, self._deployment_id)
        return deployment.inputs[input_name]

    def get_node(self, node_id):
        node = get_storage_node(self._deployment_id, node_id).to_dict()
        node['name'] = node['id']
        return Node(node)

    def get_secret(self, secret_key):
        secret = self.sm.get(models.Secret, secret_key)
        decrypted_value = cryptography_utils.decrypt(secret.value)
        return SecretType(secret_key, decrypted_value)
    ...
    def get_capability(self, capability_path):
        ...
        deployment = self.sm.get(models.Deployment, shared_dep_id)
